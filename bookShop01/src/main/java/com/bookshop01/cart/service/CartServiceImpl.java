...
public class CartServiceImpl  implements CartService{
	...
	public Map<String ,List> myCartList(CartVO cartVO) throws Exception{
		...
		if(myCartList.size()==0){ //카트에 저장된 상품이없는 경우 null을 반환
			return null;
		}
		List<GoodsVO> myGoodsList=cartDAO.selectGoodsList(myCartList); // 장바구니 페이지에 표시할 상품정보를 조회
		cartMap.put("myCartList", myCartList);
		cartMap.put("myGoodsList",myGoodsList);
		return cartMap;	// 장바구니 정보와 상품 정보를 cartMap에 저장하여 반환
	}
	
	// 테이블에 추가하기 전에 동일한 상품 번호의 갯수를 조회
